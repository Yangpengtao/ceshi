...
public class GaussianBlurFilter implements ImageFilterInterface {
	...
	void ApplyPass(float[] pixels, int width, int height, float b0, float b1, float b2, float b3, float b) {
		...
		for (int i = 0; i < height; i++) {
			...
			for (int j = steplength + 9; j < (steplength + triplewidth); j += 3) {
				pixels[j] = (b * pixels[j])
						+ ((((b1 * pixels[j - 3]) + (b2 * pixels[j - 6])) + (b3 * pixels[j - 9])) * num);
				pixels[j + 1] = (b * pixels[j + 1])
						+ ((((b1 * pixels[(j + 1) - 3]) + (b2 * pixels[(j + 1) - 6])) + (b3 * pixels[(j + 1) - 9])) * num);
				pixels[j + 2] = (b * pixels[j + 2])
						+ ((((b1 * pixels[(j + 2) - 3]) + (b2 * pixels[(j + 2) - 6])) + (b3 * pixels[(j + 2) - 9])) * num);
			}
			for (int k = ((steplength + triplewidth) - 9) - 3; k >= steplength; k -= 3) {
				pixels[k] = (b * pixels[k])
						+ ((((b1 * pixels[k + 3]) + (b2 * pixels[k + 6])) + (b3 * pixels[k + 9])) * num);
				pixels[k + 1] = (b * pixels[k + 1])
						+ ((((b1 * pixels[(k + 1) + 3]) + (b2 * pixels[(k + 1) + 6])) + (b3 * pixels[(k + 1) + 9])) * num);
				pixels[k + 2] = (b * pixels[k + 2])
						+ ((((b1 * pixels[(k + 2) + 3]) + (b2 * pixels[(k + 2) + 6])) + (b3 * pixels[(k + 2) + 9])) * num);
			}
		}
	}
	...
	float[] ConvertImageWithPadding(ImageData imageIn, int width, int height) {
	...
	public ImageData imageProcess() {
		...
		for (int i = 0; i < height; i++) {
			...
			for (int j = 0; j < width; j++) {
				int pos = (num + j) * 3;
				image.setPixelColor(j, i, (int) (imageArray[pos] * 255f), (int) (imageArray[pos + 1] * 255f),
						(int) (imageArray[pos + 2] * 255f));
			}
		}
		return image;
	}
